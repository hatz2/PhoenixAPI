...
def run():
    ...
    api.packet_manager_client.subscribe()

    try:
        while True:
            pending_send = api.packet_manager_client.get_pending_send_packets()

            for packet in pending_send:
                print(f"[SEND]: {packet.data}")

            pending_recv = api.packet_manager_client.get_pending_recv_packets()
        ...
        api.packet_manager_client.unsubscribe()
